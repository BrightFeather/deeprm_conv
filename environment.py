...
class Env:
    ...
    def observe(self):
        if self.repre == 'image':
            ...
            backlog_width = int(math.ceil(self.pa.backlog_size / float(self.pa.time_horizon * self.pa.num_res)))
            ...
            for i in xrange(self.pa.num_res):
                ...
                # print image_repr[cl_pt: cl_pt+self.pa.time_horizon, ir_pt: ir_pt + self.pa.res_slot].shape
                # print self.machine.canvas[i, :, :].shape
                # print '======================'
                image_repr[cl_pt: cl_pt + self.pa.time_horizon, ir_pt: ir_pt + self.pa.res_slot] = self.machine.canvas[
                                                                                                   i, :, :]
                ...
                for j in xrange(self.pa.num_nw):
                    ...
                    if self.job_slot.slot[j] is not None:  # fill in a block of work
                        image_repr[cl_pt: cl_pt + self.job_slot.slot[j].len,
                        ir_pt: ir_pt + self.job_slot.slot[j].res_vec[i]] = 1

                    ir_pt += self.pa.max_job_size

                cl_pt += self.pa.time_horizon

...
class Parameters:
    def __init__(self):
        ...
        self.simu_len = 10             # length of the busy cycle that repeats itself, length of each trajectory 10
        ...
        self.backlog_width = int(math.ceil(self.backlog_size / float(self.time_horizon * self.num_res)))
        self.network_input_height = self.time_horizon * self.num_res
        self.network_input_width = \
            (self.res_slot +
             self.max_job_size * self.num_nw) + \
            self.backlog_width + \
